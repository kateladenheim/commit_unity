...
public class SoundTriggerWithErrorCatch : MonoBehaviour
{
    public AudioClip sample_YES;
    public AudioClip sample_NO;
    public AudioClip errorMessage;

    private float measureClipLength;
    ...
    private void Awake()
    {
        ...
        measureClipLength = sample_YES.length + sample_NO.length;
    }

    void Update()
    {
        //only do this if both clips are longer than one second
        if (measureClipLength > 1.0f)
        {
            //if you want them one after the other
            //set up a condiition where she stops talking after reading both yes and no answers
            if (Append)
            {
                if (haveBothClipsPlayed < 2)
                {
